...
import { attachWsServer as attachModular, resolveActiveRunDir as resolveModular } from './ws-modular.js';

const USE_MODULES = process.env.WORCA_WS_MODULES !== '0';

export function attachWsServer(server, config) {
  return USE_MODULES ? attachModular(server, config) : attachLegacy(server, config);
}
