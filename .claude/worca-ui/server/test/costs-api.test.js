...
describe('GET /api/costs', () => {
  ...
  it('returns token data for runs with per-stage iter files', async () => {
    ...
    writeFileSync(join(stageDir, 'iter-1.json'), JSON.stringify({
      total_cost_usd: 0.82,
      modelUsage: {
        'claude-opus-4-6': {
          inputTokens: 14,
    ...
    expect(res.body.tokenData[runId].plan[0].cacheCreationInputTokens).toBe(56131);
    expect(res.body.tokenData[runId].plan[0].models).toEqual(['claude-opus-4-6']);
  });

  it('handles multiple stages and iterations', async () => {
    const runId = 'multi-stage';
    mkdirSync(join(dir, 'worca', 'results', runId, 'plan'), { recursive: true });
    mkdirSync(join(dir, 'worca', 'results', runId, 'implement'), { recursive: true });

    writeFileSync(join(dir, 'worca', 'results', runId, 'plan', 'iter-1.json'), JSON.stringify({
      modelUsage: { 'opus': { inputTokens: 100, outputTokens: 200, cacheReadInputTokens: 0, cacheCreationInputTokens: 0 } }
    }));
    writeFileSync(join(dir, 'worca', 'results', runId, 'implement', 'iter-1.json'), JSON.stringify({
      modelUsage: { 'sonnet': { inputTokens: 300, outputTokens: 400, cacheReadInputTokens: 50, cacheCreationInputTokens: 60 } }
    }));
    writeFileSync(join(dir, 'worca', 'results', runId, 'implement', 'iter-2.json'), JSON.stringify({
      modelUsage: { 'sonnet': { inputTokens: 500, outputTokens: 600, cacheReadInputTokens: 70, cacheCreationInputTokens: 80 } }
    }));
  ...
  it('skips malformed iter files gracefully', async () => {
    ...
    mkdirSync(join(dir, 'worca', 'results', runId, 'plan'), { recursive: true });
    writeFileSync(join(dir, 'worca', 'results', runId, 'plan', 'iter-1.json'), 'not valid json!!!');
